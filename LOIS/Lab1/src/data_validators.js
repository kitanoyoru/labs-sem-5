...
class Fact {
    ...
    isValid() {
        // p(x)={(a,0),(b,0.3),(c,1)}
        const pattern = /^[a-z]\([a-z]\)={(\([a-z],\d(\.\d)?\),)*(\([a-z],\d(\.\d)?\))?}$/;
    ...
    getTail() {
        ...
        const pairs = pairsStr.slice(1, -1).split("),(").map(pair => pair.split(","));
        return pairs[0].length === 1 ? [] : pairs.map(([key, value]) => [key, parseFloat(value)]);
    }

    isTailValid() {
        return new Set(Object.keys(this.tail)).size === Object.keys(this.tail).length;
    }
}
...
class Function {
    ...
    isValid() {
        const pattern = /^[a-z]\(([a-z],)*[a-z]\)=\([a-z]\([a-z]\)~>[a-z]\([a-z]\)\)$/;
...
Function.InvalidFunctionException = class InvalidFunctionException extends Error {
    constructor(func) {
